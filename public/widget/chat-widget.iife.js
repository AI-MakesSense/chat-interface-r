...
`)}function j(s){let e=[],t=!1,r=0,n=[],i=s.uiConfig||{},m=new H(s.relay.licenseKey||"default"),p=i.theme?.colorScheme||i.style?.theme||"light",c=p==="dark",a={branding:{companyName:i.branding?.companyName||"Support",welcomeText:i.branding?.welcomeText||i.startScreen?.greeting||"How can we help you?",firstMessage:i.branding?.firstMessage||"Hello! Ask me anything.",logoUrl:i.branding?.logoUrl},style:{theme:p,primaryColor:i.theme?.color?.accent?.primary||i.style?.primaryColor||"#0ea5e9",backgroundColor:i.theme?.color?.surface?.background||i.style?.backgroundColor||(c?"#1a1a1a":"#ffffff"),textColor:i.style?.textColor||(c?"#e5e5e5":"#1f2937"),fontFamily:i.theme?.typography?.fontFamily||i.style?.fontFamily||'-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',fontSize:i.theme?.typography?.baseSize||i.style?.fontSize||14,position:i.style?.position||"bottom-right",cornerRadius:i.style?.cornerRadius||12},features:{fileAttachmentsEnabled:i.composer?.attachments?.enabled||i.features?.fileAttachmentsEnabled||!1,allowedExtensions:i.composer?.attachments?.accept||i.features?.allowedExtensions||["jpg","jpeg","png","gif","pdf","doc","docx"],maxFileSizeKB:i.composer?.attachments?.maxSize?i.composer.attachments.maxSize/1024:i.features?.maxFileSizeKB||5e3},connection:i.connection,license:i.license,theme:i.theme,startScreen:i.startScreen,composer:i.composer},y=Q(a),v=ee(a),T=document.createElement("style");T.id="n8n-chat-widget-styles",T.textContent=`
  ...
  `,d.appendChild(w);let U=a.startScreen?.prompts||[],$=null;U.length>0&&($=document.createElement("div"),$.id="n8n-starter-prompts",$.style.cssText=`
    ...
    `,U.forEach(o=>{let l=document.createElement("button");l.className="n8n-starter-prompt",l.innerHTML=`
        ...
        <span>${o.label}</span>
      `,l.addEventListener("click",()=>{let h=document.getElementById("n8n-chat-input");h&&(h.value=o.prompt||o.label,h.focus())}),$.appendChild(l)}),d.appendChild($));let te=a.composer?.placeholder||"Type a message...",K=!!a.theme?.color?.accent,E=document.createElement("div");E.style.cssText=`
    padding: var(--cw-spacing-lg, 16px);
    padding-top: 0;
    background: var(--cw-surface-bg, ${c?"#1a1a1a":"#ffffff"});
  ...
  `,E.innerHTML=M,d.appendChild(E),a.composer?.disclaimer){let o=document.createElement("div");o.style.cssText=`
    ...
    `,o.innerHTML=`Powered by <a href="https://n8n.io" target="_blank" style="color: var(--cw-accent-primary, ${a.style.primaryColor}); text-decoration: none;">n8n</a>`,d.appendChild(o)}let I=E.querySelector("#n8n-chat-input"),se=E.querySelector("#n8n-chat-send"),O=E.querySelector("#n8n-chat-attach"),S=E.querySelector("#n8n-chat-file-input");se.addEventListener("click",()=>q()),I.addEventListener("keypress",o=>{o.key==="Enter"&&q()}),O&&S&&(O.addEventListener("click",()=>{S.click()}),S.addEventListener("change",o=>{let l=o.target.files;l&&(n=Array.from(l),console.log(`[N8n Chat Widget] Selected ${n.length} file(s)`))}));function D(){t=!t,t?(d.style.display="flex",g.style.display="none",e.length===0&&a.branding.firstMessage&&N("assistant",a.branding.firstMessage),I.focus()):(d.style.display="none",g.style.display="flex")}function N(o,l,h=!1){let u={id:`msg-${++r}`,role:o,content:l,timestamp:Date.now()};e.push(u);let f=document.createElement("div");f.id=u.id,f.style.cssText=`
    ...
    `,o==="assistant"?h?x.innerHTML=`
          <div class="n8n-typing-container">
            <div class="n8n-typing-dot"></div>
            <div class="n8n-typing-dot"></div>
            <div class="n8n-typing-dot"></div>
          </div>
        `:x.innerHTML=B(l):x.textContent=l,f.appendChild(x),w.appendChild(f),w.scrollTop=w.scrollHeight,u}function A(o,l){let h=w.querySelector(`#${o}`);if(!h)return;let u=h.querySelector("div");u&&(u.innerHTML=B(l));let f=e.find(x=>x.id===o);f&&(f.content=l),w.scrollTop=w.scrollHeight}async function q(){let o=I.value.trim();if(!o)return;N("user",o),I.value="";let l=N("assistant","",!0);try{await oe(o,l.id)}catch(h){console.error("[N8n Chat Widget] Error sending message:",h),A(l.id,"Sorry, there was an error processing your message. Please try again.")}}function re(){try{let o=new URL(window.location.href);return{pageUrl:window.location.href,pagePath:window.location.pathname,pageTitle:document.title,queryParams:Object.fromEntries(o.searchParams),domain:window.location.hostname}}catch(o){return console.error("[N8n Chat Widget] Error capturing page context:",o),{pageUrl:window.location.href,pagePath:window.location.pathname,pageTitle:document.title,queryParams:{},domain:window.location.hostname}}}async function ie(o){return new Promise((l,h)=>{let u=new FileReader;u.onload=()=>{let x=u.result.split(",")[1];l({name:o.name,type:o.type,data:x,size:o.size})},u.onerror=()=>{h(new Error(`Failed to read file: ${o.name}`))},u.readAsDataURL(o)})}async function oe(o,l){let h=s.relay.relayUrl,u=m.getSessionId();try{let f=a.connection?.captureContext!==!1,x;n.length>0&&a.features.fileAttachmentsEnabled&&(x=await Promise.all(n.map(le=>ie(le))),n=[],S&&(S.value=""));let ae=V(s,{message:o,sessionId:u,context:f?re():void 0,customContext:a.connection?.customContext,extraInputs:a.connection?.extraInputs,attachments:x}),L=await fetch(h,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(ae)});if(!L.ok)throw new Error(`HTTP ${L.status}: ${L.statusText}`);let P=await L.json(),ce=P.response||P.message||P.output||"No response received";A(l,ce)}catch(f){throw console.error("[N8n Chat Widget] Error sending message:",f),A(l,"Sorry, there was an error connecting to the server. Please try again."),f}}}var z=class{constructor(e){this.chatWindow=null;this.config=e}render(){let e=document.getElementById("chat-portal")||document.body;return this.chatWindow=this.createChatWindow({position:"fullscreen",showMinimize:!1,showHeader:this.config.portal?.showHeader??!0,headerTitle:this.config.portal?.headerTitle||this.config.branding?.companyName||"Chat"}),this.applyPortalStyles(this.chatWindow),this.addResponsiveClasses(this.chatWindow),e.appendChild(this.chatWindow),this.chatWindow.classList.add("visible"),this.chatWindow.style.display="flex",this.autoFocusInput(),this.chatWindow}createChatWindow(e){let t=document.createElement("div");t.className="chat-window",t.id="n8n-chat-window";let r=`
      background: white;
      flex-direction: column;
      overflow: hidden;
