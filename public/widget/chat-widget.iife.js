...
`;
  ...
  var md = new lib_default({
    ...
    typographer: true,
    // Enable smart quotes and other typography
    breaks: true
    // Convert \n to <br>
  });
  function renderMarkdown(markdown) {
    try {
      ...
      console.error("[N8n Chat Widget] Markdown rendering error:", error2);
      return escapeHtml2(markdown).replace(/\n/g, "<br>");
    }
  }
  function escapeHtml2(text2) {
    ...
    return text2.replace(/[&<>"']/g, (char) => map2[char]);
  }

  // widget/src/services/messaging/payload.ts
  function buildRelayPayload(runtimeConfig, options) {
    ...
    return {
      ...
      chatInput: options.message,
      // Duplicate message as chatInput for n8n compatibility
  ...
  // widget/src/utils/session-id-generator.ts
  function generateSessionId() {
    ...
    const hex = Array.from(randomValues).map((byte) => byte.toString(16).padStart(2, "0")).join("");
    return [
      hex.slice(0, 8),
      hex.slice(8, 12),
      hex.slice(12, 16),
  ...
  // widget/src/services/messaging/session-manager.ts
  var SESSION_STORAGE_PREFIX = "chat-widget-session-";
  var SESSION_START_TIME_PREFIX = "chat-widget-session-start-";
  var SessionManager = class {
    ...
    loadSession() {
      ...
      if (storedSessionId) {
        ...
        this.startTime = storedStartTime ? new Date(storedStartTime) : /* @__PURE__ */ new Date();
      }
    }
    /**
  ...
  function createChatWidget(runtimeConfig) {
    const messages = [];
    let isOpen = false;
    let messageIdCounter = 0;
    let selectedFiles = [];
    const config2 = runtimeConfig.uiConfig || {};
    const sessionManager = new SessionManager(runtimeConfig.relay.licenseKey || "default");
    const mergedConfig = {
      branding: {
        companyName: config2.branding?.companyName || "Support",
        welcomeText: config2.branding?.welcomeText || "How can we help you?",
        firstMessage: config2.branding?.firstMessage || "Hello! Ask me anything.",
        logoUrl: config2.branding?.logoUrl
      },
      style: {
        theme: config2.style?.theme || "light",
        primaryColor: config2.style?.primaryColor || "#00bfff",
        backgroundColor: config2.style?.backgroundColor || "#ffffff",
        textColor: config2.style?.textColor || "#333333",
        fontFamily: config2.style?.fontFamily || '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',
        fontSize: config2.style?.fontSize || 14,
        position: config2.style?.position || "bottom-right",
        cornerRadius: config2.style?.cornerRadius || 12
      },
      features: {
        fileAttachmentsEnabled: config2.features?.fileAttachmentsEnabled || false,
        allowedExtensions: config2.features?.allowedExtensions || ["jpg", "jpeg", "png", "gif", "pdf", "doc", "docx"],
        maxFileSizeKB: config2.features?.maxFileSizeKB || 5e3
      },
      connection: config2.connection,
      license: config2.license
    };
  ...
  `;
    ...
    const input = inputContainer.querySelector("#n8n-chat-input");
    const sendBtn = inputContainer.querySelector("#n8n-chat-send");
    const attachBtn = inputContainer.querySelector("#n8n-chat-attach");
    const fileInput = inputContainer.querySelector("#n8n-chat-file-input");
    ...
    `;
      ...
      font-size: 14px;
      line-height: 1.4;
      ${role === "user" ? `background: ${mergedConfig.style.primaryColor}; color: white;` : "background: white; color: #1f2937; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05);"}
    `;
      if (role === "assistant") {
        bubble2.innerHTML = renderMarkdown(content);
      } else {
        bubble2.textContent = content;
      }
      messageEl.appendChild(bubble2);
    ...
    function updateMessage(messageId, content) {
      ...
      if (!messageEl) return;
      const bubble2 = messageEl.querySelector("div");
      if (bubble2) {
        bubble2.innerHTML = renderMarkdown(content);
      }
    ...
    async function handleSendMessage() {
      ...
      try {
        await streamResponse(text2, assistantMessage.id);
      } catch (error2) {
        console.error("[N8n Chat Widget] Error sending message:", error2);
    ...
    function capturePageContext() {
      try {
        ...
        console.error("[N8n Chat Widget] Error capturing page context:", error2);
        return {
          pageUrl: window.location.href,
          pagePath: window.location.pathname,
          pageTitle: document.title,
    ...
    async function streamResponse(userMessage, assistantMessageId) {
      ...
      try {
        ...
        const payload = buildRelayPayload(runtimeConfig, {
          message: userMessage,
          sessionId,
          context: shouldCaptureContext ? capturePageContext() : void 0,
  ...
  // widget/src/core/portal-renderer.ts
  var PortalRenderer = class {
    constructor(config2) {
     ...
     */
    createHeader(title, showMinimize) {
      const header = document.createElement("div");
  ...
  // widget/src/core/normal-renderer.ts
  var NormalRenderer = class {
    constructor(config2) {
      ...
      this.config = config2;
      this.uiBuilder = new UIBuilder(config2);
    }
    /**
    ...
    enterFullscreen() {
      ...
      const toggleBtn = this.chatWindow.querySelector(".fullscreen-toggle-btn");
      if (toggleBtn) {
        toggleBtn.innerHTML = "\u25F2";
  ...
  // widget/src/core/config-validator.ts
  var ConfigValidator = class {
    /**
     * Validate widget configuration
     * @throws Error if configuration is invalid
     */
    static validate(config2) {
      if (config2.mode === "portal" && !config2.license) {
        throw new Error("License required for portal mode");
      }
      if (config2.connection?.webhookUrl && !this.isValidUrl(config2.connection.webhookUrl)) {
        console.error("Invalid webhook URL:", config2.connection.webhookUrl);
      }
    }
    /**
  ...
  // widget/src/core/widget.ts
  var Widget = class {
    constructor(config2) {
      this.chatWindow = null;
      ConfigValidator.validate(config2);
      this.config = config2;
      this.mode = config2.mode || "normal";
    }
    /**
  ...
  (function() {
    ...
    const __INJECT_START__ = "__START_LICENSE_FLAGS__";
    const __INJECT_END__ = "__END_LICENSE_FLAGS__";
    ...
    async function init() {
      const legacyConfig = window.ChatWidgetConfig;
      if (legacyConfig && legacyConfig.relay?.relayUrl) {
        ...
        try {
          ...
          console.error("[N8n Chat Widget] Initialization error:", error2);
          return;
        }
      }
      let licenseKey = "";
      ...
      const scriptTag = document.querySelector('script[src*="/chat-widget.js"], script[src*="/bundle.js"]');
      if (scriptTag && scriptTag.src) {
        const url = new URL(scriptTag.src);
        apiBaseUrl = url.origin;
        if (!licenseKey) {
          const match2 = url.pathname.match(/\/api\/widget\/([^\/]+)\/chat-widget\.js/);
          if (match2 && match2[1]) {
            licenseKey = match2[1];
      ...
      try {
        ...
        const runtimeConfig = {
          ...config2,
          relay: {
            relayUrl: config2.connection?.relayEndpoint || `${apiBaseUrl}/api/chat-relay`,
            widgetId: "",
            // Will be filled by relay response or not needed if relay handles it
            licenseKey
          }
        };
        createChatWidget(runtimeConfig);
      } catch (error2) {
        console.error("[N8n Chat Widget] Auto-discovery initialization error:", error2);
        if (container) {
